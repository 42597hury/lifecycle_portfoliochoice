"""Inf-horizon g=10, theta=0, Lobatto on y_1 AND xb (both Z=2.9 sigma).

Mirrors test_inf_horizon_g10_theta000_yLob29.py with:
  - n_ret_nodes_1d=(4, 5)               (K-bump on xb axis)
  - ret_lobatto_Z=(None, 2.9)           (Lobatto on xb axis only at 2.9 sigma;
                                         GH on xr)
  - state Lobatto on y_1 same as Run A: state_lobatto_Z=(None, None, 2.9)
  - n_state_quad_nodes=(4, 4, 5)        same K-bump on y_1 axis

Hypothesis follow-on to Run A: if Lobatto on y_1 alone partially tames
the corner instability, adding Lobatto on xb (the bond return innovation
itself, not just its predictor y_1) should remove the residual corner
oscillation by anchoring the bond return tails at +/- 2.9 sigma.

Wall budget: Run A is the closest reference. n_ret_quad doubles 16 -> 20
(4*5 vs 4*4); state-quad joint nodes unchanged. Per-iter wall scales
roughly with n_state * n_state_quad * n_ret_quad -> ~25% slower than Run A.
"""
import os
import time
...
disc = DiscretizationConfig(
    ...
    n_eps_nodes=4,
    n_eta_nodes=3,
    n_ret_nodes_1d=(4, 5),              # K-bump on xb (4 for xr, 5 for xb)
    ret_lobatto_Z=(None, 2.9),          # Lobatto on xb at Z=2.9 sigma
    n_state_quad_nodes=(4, 4, 5),       # K-bump on y_1
    state_lobatto_Z=(None, None, 2.9),  # Lobatto on y_1 at Z=2.9 sigma
)
...
print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=10 (theta={TERM_PREMIUM_THETA}, Lobatto y_1+xb Z=2.9)", flush=True)
print(f"  state_grid=(10,10,10), state_n_stds=(3,3,3), n_z=1, max_iter=10", flush=True)
print(f"  n_ret=(4,5), ret_lobatto_Z=(None,2.9)", flush=True)
...
BUNDLE_NAME = f"test_inf_horizon_g10_theta{int(TERM_PREMIUM_THETA*100):03d}_yLob29_xbLob29"
...
run_config_snapshot = {
    ...
    "predictability_ablation": {
        ...
        "system_title": f"Full System (real, term_premium_theta={TERM_PREMIUM_THETA}, Lobatto y_1+xb Z=2.9)",
    ...
    "ablation_axis": "lobatto_y1_xb_Z29",
}
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=10 theta=0 yLob29+xbLob29 run", flush=True)
print("=" * 70, flush=True)
