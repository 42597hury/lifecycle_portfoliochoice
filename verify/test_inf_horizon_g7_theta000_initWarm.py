"""Inf-horizon g=7, theta=0, warm-start init from g10 retire-only theta=0.

Identical to test_inf_horizon_g7.py except the init scalars match the
late-retirement center-cell policy from test_retire_g10_theta000.py:

  Live probe at z=median, state-center (5,5,5), SCF-median W, age 67:
    alpha_s = 1.188
    alpha_b = -0.567
    bill    =  0.379

So init_alpha_s, init_alpha_b are set to roughly that — rounded to (1.0, -0.3)
to keep the change a clean diagnostic without overfitting.

Hypothesis: the previous (0.85, 0.44) init plants the IH iteration far
from the theta=0 fixed point (alpha_b should be near 0 to negative, not
+0.44). With damping=1.0 the iteration overshoots into corners; with
damping=0.5 it cycles but doesn't converge. Warm init tests whether the
divergence is driven primarily by being on the wrong side of the manifold.

Compare against:
  - test_inf_horizon_g7_theta000        (init=(0.85, 0.44), damp=1.0, stop=4.82)
  - test_inf_horizon_g7_theta000_damp05 (init=(0.85, 0.44), damp=0.5, stop=4.67)
"""
import os
import time
...
# CHANGED FROM BASELINE: init scalars matched to g10 retire-only theta=0
# late-retirement center-cell policy (was 0.85, 0.44).
sc = SolverConfig(
    tol=1e-6,
    max_iter=10,
    max_backtrack_iter=5,
    init_alpha_s=1.0,                   # was 0.85
    init_alpha_b=-0.3,                  # was 0.44 (now matches theta=0 expectation)
...
IH_TOL = 1e-5
IH_MAX_ITER = 50
IH_DAMPING = 1.0                        # back to baseline damping
IH_PROGRESS_EVERY = 1
TERM_PREMIUM_THETA = 0.0


print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=7 (theta={TERM_PREMIUM_THETA}, init warm-start)", flush=True)
print(f"  init=(alpha_s={sc.init_alpha_s}, alpha_b={sc.init_alpha_b})  damping={IH_DAMPING}", flush=True)
print("=" * 70, flush=True)
...
BUNDLE_NAME = f"test_inf_horizon_g7_theta{int(TERM_PREMIUM_THETA*100):03d}_initWarm"
...
run_config_snapshot = {
    ...
    "predictability_ablation": {
        ...
        "system_title": f"Full System (real, theta={TERM_PREMIUM_THETA}, init=(1.0,-0.3))",
    ...
    "ablation_axis": "init_warm_from_g10_retire_theta0",
}
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=7 theta=0 init-warm run", flush=True)
print("=" * 70, flush=True)
