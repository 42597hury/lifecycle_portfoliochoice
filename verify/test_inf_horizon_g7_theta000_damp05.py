"""Inf-horizon g=7, theta=0, damping=0.5.

Identical to test_inf_horizon_g7.py except IH_DAMPING=0.5 (was 1.0).
Diagnostic test: does outer-iteration damping break the corner alpha_b
oscillation that survived theta=0, state-grid refinement (g7->g12), and
quadrature refinement (Lobatto on y_1 and xb at g10)?

Hypothesis: the corner instability is structural in the IH update map.
With damping=0.5, each iter's policy is 0.5*(old) + 0.5*(new), so per-iter
oscillation amplitude is halved and a non-decaying limit cycle should
collapse to a (slower-converging but stable) trajectory.

Wall budget: g7 theta=0 GH took 0.81 min for 50 iters. Damping changes
no compute per iter; same wall. May converge in fewer iters, ending early.

Compare against:
  - test_inf_horizon_g7_theta000  (damping=1.0, diverged stop=4.82)
"""
import os
import time
...
IH_TOL = 1e-5
IH_MAX_ITER = 50
IH_DAMPING = 0.5                        # halved from 1.0
IH_PROGRESS_EVERY = 1
TERM_PREMIUM_THETA = 0.0


print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=7 (theta={TERM_PREMIUM_THETA}, damping={IH_DAMPING})", flush=True)
...
BUNDLE_NAME = f"test_inf_horizon_g7_theta{int(TERM_PREMIUM_THETA*100):03d}_damp05"
...
run_config_snapshot = {
    ...
    "predictability_ablation": {
        ...
        "system_title": f"Full System (real, term_premium_theta={TERM_PREMIUM_THETA}, damping=0.5)",
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=7 theta=0 damping=0.5 run", flush=True)
print("=" * 70, flush=True)
