"""Inf-horizon g=10, theta=0, Lobatto on y_1 (Z=2.9 sigma).

Mirrors test_inf_horizon_g7.py / test_inf_horizon_g12.py with:
  - state_grid_sizes=(10,10,10)         (1000 cells, between g7=343, g12=1728)
  - n_state_quad_nodes=(4, 4, 5)        (K-bump on y_1 axis, the bond-return
                                         refinement axis where M = Sigma_rs
                                         Sigma_ss^-1 is dominated)
  - state_lobatto_Z=(None, None, 2.9)   (Lobatto on y_1 axis only at 2.9 sigma;
                                         GH on cape, spr)

Hypothesis: g7 (343 cells) and g12 (1728 cells) both diverged at theta=0
with stop sup-norm 4.82 and 8.40 respectively. The bond corner-cell
instability survived even when the level term premium was zeroed; we now
test whether finer-and-tail-anchored y_1 quadrature damps the conditional
bond-return tails feeding the corner alpha_b oscillation.

Wall budget: g12 was 3.57 min @ chunks=8 (1728 cells, GH state quad 4^3=64).
g10 has 1000 cells (0.58x g12), state-quad joint nodes 4*4*5=80 (1.25x g12's
64) -> roughly 0.58 * 1.25 = 0.72x g12 -> ~2.6 min on A100 worst case.
"""
import os
import time
...
disc = DiscretizationConfig(
    ...
    n_eps_nodes=4,
    n_eta_nodes=3,
    n_ret_nodes_1d=(4, 4),              # GH on returns (xr, xb)
    ret_lobatto_Z=None,
    n_state_quad_nodes=(4, 4, 5),       # K-bump on y_1
    state_lobatto_Z=(None, None, 2.9),  # Lobatto on y_1, prescribed Z=2.9 sigma
)
...
sc = SolverConfig(
    ...
    cell_vmap_chunks=8,                 # match g12 — A100-40GB safe
...
print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=10 (theta={TERM_PREMIUM_THETA}, Lobatto y_1 Z=2.9)", flush=True)
print(f"  state_grid=(10,10,10), state_n_stds=(3,3,3), n_z=1, max_iter=10", flush=True)
print(f"  n_state_quad=(4,4,5), state_lobatto_Z=(None,None,2.9)", flush=True)
print("=" * 70, flush=True)
...
BUNDLE_NAME = f"test_inf_horizon_g10_theta{int(TERM_PREMIUM_THETA*100):03d}_yLob29"
...
run_config_snapshot = {
    ...
    "predictability_ablation": {
        ...
        "system_title": f"Full System (real, term_premium_theta={TERM_PREMIUM_THETA}, Lobatto y_1 Z=2.9)",
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=10 theta=0 yLob29 run", flush=True)
print("=" * 70, flush=True)
