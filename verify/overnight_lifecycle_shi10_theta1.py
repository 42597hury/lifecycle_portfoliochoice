"""Overnight lifecycle solve — App-C Shi-10 theta=1, full 78 ages.

Identical to overnight_lifecycle_shi10_theta0.py except TERM_PREMIUM_THETA=1.0.
Queued to run after theta=0 completes.
...
solve_control = SolveControl(
    checkpoint_every_n_ages=1, save_on_interrupt=True, return_partial_on_interrupt=True,
)
...
print(f"Setup wall: {time.time() - t0:.1f}s  n_age={pc.n_age} n_z={pc.n_z} N_state={pc.N_state} n_w={pc.n_w}", flush=True)

t0 = time.time()
C, S, B, diag = run_lifecycle_solver(model, pc, sc, verbose=1, solve_control=solve_control)
solve_wall = time.time() - t0
print(f"\nSolve wall: {solve_wall:.1f}s = {solve_wall / 3600:.2f} hr", flush=True)
