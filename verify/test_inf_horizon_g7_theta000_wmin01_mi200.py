"""Inf-horizon g=7, theta=0, wealth_min=0.10, IH_MAX_ITER=200.

Continuation of test_inf_horizon_g7_theta000_wmin01.py — same config
except IH_MAX_ITER bumped from 50 to 200 to confirm convergence to
IH_TOL=1e-5.

Previous wmin=0.10 result at IH_MAX_ITER=50:
  - Stop monotonically decreasing iter 4 onward
  - Per-iter contraction rate ~0.95
  - Final stop = 2.85e-3 (285x above IH_TOL=1e-5)
  - Projected: ~110 more iters needed at observed rate

Wall: ~50 iters per ~46s, so 200 iters ~3 min.
"""
import os
import time
...
IH_TOL = 1e-5
IH_MAX_ITER = 200                       # bumped from 50
IH_DAMPING = 0.5
IH_PROGRESS_EVERY = 10                  # less verbose
TERM_PREMIUM_THETA = 0.0


print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=7 (theta={TERM_PREMIUM_THETA}, wmin=0.10, MI=200)", flush=True)
print("=" * 70, flush=True)
...
BUNDLE_NAME = f"test_inf_horizon_g7_theta{int(TERM_PREMIUM_THETA*100):03d}_wmin01_mi200"
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=7 theta=0 wmin=0.10 MI=200 run", flush=True)
print("=" * 70, flush=True)
