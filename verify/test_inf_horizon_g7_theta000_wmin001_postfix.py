"""Inf-horizon g=7, theta=0, wealth_min=0.01, IH_MAX_ITER=200 — POSTFIX (v2).

Mirrors test_inf_horizon_g7_theta000_wmin01_mi200.py except:
  - wealth_min: 0.10 -> 0.01 (the canonical lifecycle wealth floor)
  - SolverConfig.max_iter: 10 -> 50 (per project memory; Newton inner cap,
    distinct from IH outer cap which stays at 200)

This is the kink-fix correctness gate at wmin=0.01 — handoff criterion 1 from
docs/handoff/HANDOFF_IH_LIFT_KINK_STRUCTURAL_FIX.md.

Pre-fix baseline at wmin=0.01 (test_inf_horizon_g7_theta000):
  Stop=4.82, alpha_b in [-6.19, 8.82] — diverges.

Post-fix targets (validation criteria):
  - Final stop sup-norm < 1e-4 (matches the wmin=0.10 reference at 6.95e-5).
  - alpha_b range bounded in [-3, +3].
  - Stop trajectory monotonically decreasing from iter 1 onward.
  - alpha_b at low-W indices (W idx 3-7) NOT extreme (|alpha_b| < 5 throughout).

v2 vs v1: this exact configuration converged neither pre-fix nor under v1
(commit 2d33c95, reverted at 5dafd0b). The v1 global-max floor was also wrong
at wmin=0.10 — see docs/handoff/HANDOFF_KINK_FIX_REGRESSION_AT_WMIN_010.md.
v2 keeps the floor per-cell with a 0-sentinel for cells without a constrained
band, so wmin=0.10 stays clean while wmin=0.01 gets the targeted clip.

Reference for cross-bundle compare (wmin=0.10 baseline; unchanged by v2):
  saved_runs/inf_horizon/test_inf_horizon_g7_theta000_wmin01_mi200/
"""
import os
import time
...
disc = DiscretizationConfig(
    n_wealth=100,
    wealth_min=0.01,                    # CANONICAL lifecycle wealth floor (was 0.10)
    wealth_max=750.0,
    n_savings=100,
...
sc = SolverConfig(
    tol=1e-6,
    max_iter=50,                        # Newton inner cap, per project memory
...
print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=7 (theta={TERM_PREMIUM_THETA}, wmin=0.01, MI=200) POSTFIX v2", flush=True)
print("=" * 70, flush=True)
...
B_arr = np.asarray(B)
print(
    f"\n  alpha_b @ low-W idx 3-7 (handoff criterion 1.4): "
    f"max |B[..., 3:8]| = {float(np.abs(B_arr[..., 3:8]).max()):.3f} (target < 5.0)",
    flush=True,
)


stop_hist = np.maximum(
    diag.get("xi_supnorm_history", np.array([])),
    diag.get("share_supnorm_history", np.array([])),
)
if stop_hist.size > 1:
    diffs = np.diff(stop_hist)
    n_increases = int((diffs > 0).sum())
    print(
        f"\n  Stop-trajectory monotonicity (criterion 1.3): "
        f"{n_increases} non-decreasing transitions out of {diffs.size} (target ~0)",
        flush=True,
    )
...
if nih is not None:
    print(
        ...
        f"p99={nih['p99']:.0f}  max={nih['max']}  (cap={sc.max_iter})",
        flush=True,
    )
...
BUNDLE_NAME = f"test_inf_horizon_g7_theta{int(TERM_PREMIUM_THETA*100):03d}_wmin001_postfix"
...
run_config_snapshot = {
    ...
    "predictability_ablation": {
        ...
        "system_title": f"Full System (real, theta={TERM_PREMIUM_THETA}, wmin=0.01, MI=200) POSTFIX v2",
    ...
    "ablation_axis": "wealth_min_001_postfix_kink_floor_v2",
    "fix": "lift_kink_w_floor_v2_per_cell",
}
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=7 theta=0 wmin=0.01 MI=200 POSTFIX v2 run", flush=True)
print("=" * 70, flush=True)
