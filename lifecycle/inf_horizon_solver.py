...
def _compute_w_floor_from_policy(C_old: np.ndarray, wealth_grid: np.ndarray) -> float:
    ...
    exactly at constrained-corner cells (the ``jnp.where`` clamp on line 1413 of
...
def run_infinite_horizon_solver(
    ...
    progress_probe_state_idx: int | None = None,  # None → per-axis state center via ravel_multi_index
    ...
    try:
        for it in range(max_iter):
            ...
            # Open-question 2: start global, refine if needed). w_floor=0
            # at iter 0 if cold-start has no constrained band; proper floor
            # kicks in once a lifted policy has been produced.
