"""Inf-horizon test at state_grid=(9,9,9), state_n_stds=(3,3,3).

Mirrors test_retire_g9.py's disc + solver config but runs the inf-horizon
fixed-point Bellman iteration instead of lifecycle backward induction.
n_z=1 (inf-horizon convention: pension=0, psi=1 → z is inert).

Tests that all the recent fixes (Variant B per-savings warm-start,
per-savings FOC normalization, failed-cell neighbor seed) carry through
the inf-horizon path. Inf-horizon shares the retirement kernel via
_build_per_age_retirement_kernel; same solver internals get exercised.

Wall budget:
  ~16s/age x ~30 IH iters (typical convergence at IH_TOL=1e-5)
  = ~8 min on A100. With IH_MAX_ITER=50 safety cap, worst-case ~13 min.

Compare against:
  - saved_runs/test_retire_g9/  (lifecycle retirement-only baseline)
"""
import os
import time
...
print("=" * 70, flush=True)
print("TEST INF-HORIZON g=9  —  state_grid=(9,9,9), state_n_stds=(3,3,3), n_z=1, max_iter=10", flush=True)
print("=" * 70, flush=True)
...
BUNDLE_NAME = "test_inf_horizon_g9"
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=9 run", flush=True)
print("=" * 70, flush=True)
