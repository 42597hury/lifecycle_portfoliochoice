"""Inf-horizon g=7, theta=0, wealth_min=0.01, IH_MAX_ITER=200 — POSTFIX validation.

Mirrors test_inf_horizon_g7_theta000_wmin01_mi200.py except:
  - wealth_min: 0.10 -> 0.01 (the canonical lifecycle wealth floor)
  - SolverConfig.max_iter: 10 -> 50 (per project memory project_ih_newton_maxiter_50;
    Newton inner cap, distinct from IH outer cap which stays at 200)

This is the validation run for the lift-kink V'(W') quadrature floor introduced
in lifecycle/solver.py:retirement_foc_jac_ccv (parameter `w_floor`) and wired
into the IH outer loop in lifecycle/inf_horizon_solver.py via the per-iter
`_compute_w_floor_from_policy(C_old, wealth_grid)` helper.

Pre-fix baseline at wmin=0.01 (test_inf_horizon_g7_theta000):
  Stop=4.82, alpha_b in [-6.19, 8.82] — diverges (handoff Table).

Post-fix expected (handoff validation criteria):
...
Reference for cross-bundle compare:
...
print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=7 (theta={TERM_PREMIUM_THETA}, wmin=0.01, MI=200) POSTFIX", flush=True)
print("=" * 70, flush=True)
...
# Validation criterion 1.4: alpha_b at low-W indices (W idx 3-7) NOT extreme.
S_arr = np.asarray(S)
B_arr = np.asarray(B)
print(
...
# Validation criterion 1.3: stop trajectory monotonicity.
...
run_config_snapshot = {
    ...
    "predictability_ablation": {
        ...
        "system_title": f"Full System (real, theta={TERM_PREMIUM_THETA}, wmin=0.01, MI=200) POSTFIX",
    ...
    "ablation_axis": "wealth_min_001_postfix_kink_floor",
    "fix": "lift_kink_w_floor",
}
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=7 theta=0 wmin=0.01 MI=200 POSTFIX run", flush=True)
print("=" * 70, flush=True)
