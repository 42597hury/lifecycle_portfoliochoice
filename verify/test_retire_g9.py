"""Retirement-only test at state_grid=(9,9,9), n_z=10, max_iter=10.

One step up from g7. Odd-N keeps the per-axis-mid probe exactly on the
unconditional state mean (mid index = 4 lands at u=0). 729 state cells,
2.13x more than g7's 343.

Carries forward the production-config wins:
  - max_iter=10 (Variant B warm-start makes Newton converge in 1-2 iters
                 typical, p99=4 at g7)
  - use_line_search=True (canonical default; the line-search-off flag is
                          being fixed by another agent and shouldn't be
                          relied on right now)
  - max_backtrack_iter=5

Wall estimate (extrapolating from g7 mi10 wall of 4.62 min):
  4.62 min x (729/343) = ~9.8 min
"""
import os
import time
...
disc = DiscretizationConfig(
    ...
    wealth_max=750.0,
    n_savings=100,
    state_grid_sizes=(9, 9, 9),         # 729 cells (vs 343 at g=7) — odd per axis
...
print("=" * 70, flush=True)
print("TEST g=9 RETIRE-ONLY  —  state_grid=(9,9,9), state_n_stds=(3,3,3), n_z=10, max_iter=10", flush=True)
print("=" * 70, flush=True)
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  retirement-only g=9 run", flush=True)
print("=" * 70, flush=True)
print(f"  Solve wall: {solve_wall / 60:.2f} min", flush=True)
print(f"  Bundle:     {BUNDLE_DIR}/", flush=True)
print(f"  Compare against: saved_runs/test_retire_g7_mi10_nols/  (g=7, max_iter=10, 4.62 min)", flush=True)
