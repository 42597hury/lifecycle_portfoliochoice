...
### C3.2 Sigma_r_cond vs Sigma_rr — the load-bearing choice — **AMENDED 2026-05-06**

**This section's original conclusion was WRONG. Revisited and inverted: the kernel was using `Sigma_r_cond` and that is the bug; CCV w8566 eq. (10) prescribes the unconditional innovation covariance `Sigma_rr`. Patched in `precompute.py:235-237` on 2026-05-06.**
...
#### Why the original audit got this wrong

The original audit treated the partial-out `eps_r = v_r − M v_s` as if it redefined the σ² that enters eq. (10). It does not. The partial-out is an internal step in the *quadrature* — `discretization.py` uses `Sigma_r_cond` as the Cholesky factor for the inner integral over `eps_r` conditional on `v_s`. The eq.-(10) constants are constants of the *expectation* in CCV's derivation, which is taken over the entire innovation `v_{t+1}`, with no sub-decomposition. The σ² that pops out of the second-order expansion of `log E[α_f exp(r_f) + α_s exp(r_f + xr) + ...]` is the variance of `(xr, xb)` at the level of the eq.-(5) `Sigma_v`, i.e. `Sigma_rr[1:, 1:]`.

#### Numerical clincher — CCV Table 2

CCV w8566 Table 2 reports (cross-correlations of VAR residuals × 100, plus diagonal s.d.s):

```
sigma(xr)  ≈  7.75% per quarter  ≈  15.5% per year
sigma(xb)  ≈  3.8%  per quarter  ≈   7.6% per year
```

Our model:

```
sqrt(Sigma_rr[1,1])     = 0.159 = 15.9% / yr   ← matches CCV Table 2 within 3%
sqrt(Sigma_r_cond[1,1]) = 0.031 =  3.1% / yr   ← off by ~5×

sqrt(Sigma_rr[2,2])     = 0.076 =  7.6% / yr   ← matches CCV Table 2 exactly
sqrt(Sigma_r_cond[2,2]) = 0.023 =  2.3% / yr   ← off by ~3×
```

CCV's framework consumes the unconditional matrix; only that reading reproduces their reported magnitudes. That is the smoking gun.

#### Markowitz cross-check

At γ=5, the Merton α_s for a one-period optimization with eq.-(10) Σ_xx is:

```
α_s* = (mu_xr + sigma2_xr/2) / (gamma * sigma2_xr)
     ≈ 0.54  with Sigma_rr[1,1]    (matches CCV Table 3 of ~0.5)
     ≈ 11.6  with Sigma_r_cond[1,1] (5× past the leverage cap)
```

Eleven-six is implausible and is exactly the reading the original
production solver produced in retire-only smoke runs.

#### Implementation impact

The patch (commit on 2026-05-06) changes `precompute.py:235-237` to read from `Sigma_rr` instead of `Sigma_r_cond`. The diagnostics (`_diag_euler_errors.py` and family) auto-propagate via `pc.sigma2_xr` so no kernel-side rewiring is needed. Bundles saved before the patch carry the old (smaller) σ² constants; the new bundle metadata field `ccv_sigma_choice` distinguishes pre/post-patch bundles so downstream consumers can refuse to mix.
