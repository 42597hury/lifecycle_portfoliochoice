"""Inf-horizon g=7, theta=0, Newton max_iter=50, damping=0.5.

Combo run targeting two noise sources for the outer-loop oscillation:
  - Newton max_iter=50 (was 10): per-iter Newton histogram showed
    p99=8, max=10 (cap-bound). Cells needing 11-20 iters were failing
    and falling back to neighbor-seed, propagating policy noise.
  - Outer damping=0.5 (was 1.0): convex-combine new and old policy
    iter-to-iter to suppress mid-iter overshoots.

Expected wall: ~0.8 min @ g7 (Newton max_iter=50 increases per-iter
worst-case but most cells still solve in 1-3 Newton iters; only the
corner-cell tails see additional iters).

Compare against:
  - test_inf_horizon_g7_theta000        (mi=10, damp=1.0, stop=4.82)
  - test_inf_horizon_g7_theta000_damp05 (mi=10, damp=0.5, stop=4.67)
  - test_inf_horizon_g7_theta000_initWarm (mi=10, damp=1.0, init warm, stop=6.90)
"""
import os
import time
...
sc = SolverConfig(
    tol=1e-6,
    max_iter=50,                        # was 10 — IH Newton cap rule (memory: project_ih_newton_maxiter_50)
...
IH_TOL = 1e-5
IH_MAX_ITER = 50
IH_DAMPING = 0.5                        # outer-loop damping
IH_PROGRESS_EVERY = 1
TERM_PREMIUM_THETA = 0.0


print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=7 (theta={TERM_PREMIUM_THETA})  mi=50, damp=0.5", flush=True)
print(f"  state_grid=(7,7,7), state_n_stds=(3,3,3), n_z=1", flush=True)
print("=" * 70, flush=True)
...
BUNDLE_NAME = f"test_inf_horizon_g7_theta{int(TERM_PREMIUM_THETA*100):03d}_mi50_damp05"
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=7 theta=0 mi=50 damp=0.5 run", flush=True)
print("=" * 70, flush=True)
