"""Retirement-only test at state_grid=(11,11,11), n_z=10, max_iter=10.

One step up from g9. Odd-N keeps probe on mu_s exactly. 1331 state cells
(vs 729 at g=9), 1.83x more.

Carries forward the production-config wins: max_iter=10, use_line_search=True,
max_backtrack_iter=5, gather_precision='f32'.

Wall estimate: 9.23 min x 1.83 = ~16.9 min on A100.
"""
import os
import time
...
disc = DiscretizationConfig(
    ...
    wealth_max=750.0,
    n_savings=100,
    state_grid_sizes=(11, 11, 11),      # 1331 cells (vs 729 at g=9) — ODD per axis
...
print("=" * 70, flush=True)
print("TEST g=11 RETIRE-ONLY  —  state_grid=(11,11,11), state_n_stds=(3,3,3), n_z=10, max_iter=10", flush=True)
print("=" * 70, flush=True)
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  retirement-only g=11 run", flush=True)
print("=" * 70, flush=True)
print(f"  Solve wall: {solve_wall / 60:.2f} min", flush=True)
print(f"  Bundle:     {BUNDLE_DIR}/", flush=True)
print(f"  Compare against: saved_runs/test_retire_g9/  (g=9, mi=10, 9.23 min)", flush=True)
