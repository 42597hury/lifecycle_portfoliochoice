"""Inf-horizon g=7, theta=0, Newton mi=10, damping=0.1.

Continuation of damp=0.2 success. damp=0.2 broke the iter-20+ explosion
(stop dropped 4.82 -> 1.98, alpha_b range narrowed [-8.23, 6.95] vs
baseline [-6.19, 8.82]) but a residual oscillation persists. Try
damping=0.1 (1/10 step) to see if the residual cycle damps to true
convergence.

Wall: ~0.8 min @ g7.

Compare:
  - g7 baseline       : d=1.0, stop=4.82
  - g7 damp=0.5       : d=0.5, stop=4.67
  - g7 damp=0.2       : d=0.2, stop=1.98, alpha_b in [-8.23, 6.95]
"""
import os
import time
...
sc = SolverConfig(
    tol=1e-6,
    max_iter=10,                        # mi=10 keeps Newton-cap soft regularizer
...
IH_TOL = 1e-5
IH_MAX_ITER = 50
IH_DAMPING = 0.1                        # one-tenth step
IH_PROGRESS_EVERY = 1
TERM_PREMIUM_THETA = 0.0


print("=" * 70, flush=True)
print(f"TEST INF-HORIZON g=7 (theta={TERM_PREMIUM_THETA})  mi=10, damp=0.1", flush=True)
print("=" * 70, flush=True)
...
BUNDLE_NAME = f"test_inf_horizon_g7_theta{int(TERM_PREMIUM_THETA*100):03d}_mi10_damp01"
...
run_config_snapshot = {
    ...
    "predictability_ablation": {
        ...
        "system_title": f"Full System (real, theta={TERM_PREMIUM_THETA}, mi=10, damp=0.1)",
    ...
    "ablation_axis": "newton_mi10_damp01",
}
...
print("\n" + "=" * 70, flush=True)
print("DONE  —  inf-horizon g=7 theta=0 mi=10 damp=0.1 run", flush=True)
print("=" * 70, flush=True)
