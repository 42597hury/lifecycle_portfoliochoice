...
def term_premium_theta_label(theta):
    """File-safe theta label matching the term-premium scaling data builder."""
    return f"{float(theta):.2f}".replace("-", "m").replace(".", "p")


def term_premium_theta_csv_path(theta, base_path=DEFAULT_CSV_PATH):
    """Return the VAR CSV path for a yield-stage term-premium theta dataset.

    ``theta=None`` returns the active empirical baseline. Numeric theta values
    expect files generated by ``data/build_var_dataset_term_premium_scale.py``.
    """
    if theta is None:
        return str(base_path)
    label = term_premium_theta_label(theta)
    return f"data/term_premium_scaling/var_dataset_theta_{label}.csv"


def ccv_nominal_theta_csv_path(theta, base_path=DEFAULT_CSV_PATH):
    """Return the VAR CSV path for the CCV nominal-yield theta datasets.

    ``theta=None`` returns the active empirical baseline. Numeric theta values
    expect files generated by
    ``data/build_var_dataset_ccv_nominal_yield_theta.py``.
    """
    if theta is None:
        return str(base_path)
    label = term_premium_theta_label(theta)
...
def build_real_full_var_config_term_premium_theta(
    theta,
    trend="c",
    estimation="restricted",
):
    """Full real-yield VAR using a generated term-premium-theta dataset."""
    csv_path = term_premium_theta_csv_path(theta)
    var_config, fit_obj, data = build_real_full_var_config(
        csv_path=csv_path,
        trend=trend,
        estimation=estimation,
    )
    var_config["term_premium_theta"] = None if theta is None else float(theta)
    return var_config, fit_obj, data


def build_real_full_var_config_ccv_nominal_theta(
    theta,
    trend="c",
    estimation="restricted",
):
    """Full real-yield VAR using a CCV nominal-yield theta dataset."""
    csv_path = ccv_nominal_theta_csv_path(theta)
    var_config, fit_obj, data = build_real_full_var_config(
        csv_path=csv_path,
        trend=trend,
        estimation=estimation,
    ...
    var_config["term_premium_theta"] = None if theta is None else float(theta)
    var_config["term_premium_dataset"] = "ccv_nominal_yield_scaling"
    return var_config, fit_obj, data
